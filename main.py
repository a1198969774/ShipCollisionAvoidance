...
# 动作指令集---> v,w
action_dict = {0: [-35], 1: [-30], 2: [-25], 3: [-20], 4: [-15],
               5: [-10], 6: [-5], 7: [0], 8: [5], 9: [10],
               10: [15], 11: [20], 12: [25], 13: [30], 14: [35]}
...
class DQN:
    def __init__(self):
        ...
        self.Epsilon = 1
        self.choice = 'cross_left'  # 获取选择
        self.load_path = '/home/NAVISKY/oau01/disk/saved_networks/cross_left'
        #self.load_path = '/home/NAVISKY/oau01/disk/saved_networks/10_D3QN_PER_image_add_sensor_obstacle_world_30m_'
        # 静态障碍下训练好的模型
        self.load_date = '2021-02-06'
        self.path = ""
        ...
        create_network_fn = create_model
        # Initialize Network
        self.output = create_model(4, self.input_shape,self.num_actions,
                                             'eval_model',create_network_fn,trainable=True,noisy=None)
        self.output_target = create_model(4, self.input_shape,self.num_actions,
                                             'eval_model',create_network_fn,trainable=True,noisy=None)
    ...
    def init_sess(self):
        ...
        if check_save == 1:
            ...
            saver.restore(sess, self.load_path + self.load_date + "/model.ckpt")
    ...
    def save_model(self):
        ...
        save_path = self.saver.save(self.sess,
                                    self.path + self.date_time + "/model.ckpt")
    ...
    def main(self):
        ...
        # 随机初始化起点和终点的位置
        randposition = []
        position_start = np.random.random_sample(2) * 2 * 2000 - 2000
        randposition.append(position_start)
        position_goal = []
        # 随机初始化起点和终点的位置
        # b = str(3)
        # a = str(self.date_time)
        while (True):
            randaangle = np.random.random_sample(1) * 2 * math.pi
            position_goal_x = position_start[0] + math.cos(randaangle) * self.d
            position_goal_y = position_start[1] + math.sin(randaangle) * self.d
            if position_goal_x > -2500.0 and position_goal_x < 2500.0 and position_goal_y > -2500.0 and position_goal_y < 2500.0:
                position_goal.append(position_goal_x)
                position_goal.append(position_goal_y)
                randposition.append(position_goal)
                break

        env.reset_env(self.choice,
                      start=[
                          randposition[0][0], randposition[0][1]], goal=[
                randposition[1][0], randposition[1][1]])

        env_info, jackal_x_temp, jackal_y_temp, radar_result, image_result, heading, jackal0_x_temp, jackal0_y_temp = env.get_env()
        # 动态障碍的坐标
        jackal0_x = []
        jackal0_y = []
        step_for_newenv = 0
        ...
        radar_list.append(radar_result)
        ...
        while True:
            ...
            cmd = [0.0, 0.0]
            v_cmd = SPEED
            omiga_cmd = action_dict[action_in][0]
            cmd[0] = v_cmd
            cmd[1] = omiga_cmd
            saved_cmd = cmd
            roll = env.step(cmd)
            ...
            terminal = env_info[-3]  # 获取terminal
            reward = env_info[-2]  # 获取reward
            saved_cmd.append(env_info[-1])
            ...
            self.Experience_Replay(
                ...
                terminal)
            if reward >= 10:
                record_reward_list.append(0.2)
                total_reward += 1
            else:
                record_reward_list.append(reward)

            reward_mixture_list.append(env_info[-1])
            if self.progress == 'Training':
                ...
                saved_cmd.append(self.loss)
                record_loss_list.append(self.loss)
            ...
            if terminal:
                #if self.score>=30 or self.episode % 5 == 0:
                if self.score >= 30 or self.episode % 5 == 0 and self.progress == 'Training':
                #if self.episode % 5 == 0:
                    # if self.progress == 'Trai
                    # ning' and step_for_newenv < 180 and self.score >20:
                    path = '/home/NAVISKY/oau01/disk/saved_results/' + '_' + self.choice + '_' + str(
                        self.date_time) + '/' + str(self.episode) + '_' + str(step_for_newenv)
                    if not os.path.exists(path):
                        os.makedirs(path)
                    dynamic_x = []
                    dynamic_x.append(jackal0_x)
                    dynamic_y = []
                    dynamic_y.append(jackal0_y)

                    str2 = []
                    str1 = "start_position:" + str(randposition[0]) + ",goal_position:" + str(randposition[1])
                    str2.append(str1)
                    np.savetxt(path + '/init.txt', str2, fmt="%s")
                    np.savetxt(path + '/path.txt', zip(zip(jackal_x, jackal_y), heading_list), fmt="%s", delimiter=',')
                    np.savetxt(path + '/dynamic_path.txt', zip(jackal0_x, jackal0_y), fmt="%s", delimiter=',')
                    np.savetxt(path + '/radar.txt', radar_list, fmt="%s", delimiter=',')
                    np.savetxt(path + '/image.txt', [img.flatten() for img in image_list], fmt="%s", delimiter=',')
                    np.savetxt(path + '/reward.txt', action_list, fmt="%s", delimiter=',')
                    np.savetxt(path + '/roll.txt', roll_list, fmt="%s", delimiter=',')
                radar_list = []
                roll_list = []
                image_list = []
                jackal_x = []
                ...
                # Print informations
                print('step:' + str(
                    self.step) + '/' + 'step_new:' + str(step_for_newenv) + '/' + 'episode:' + str(
                    total_reward) + '/' + str(
